...
category: ERC
created: 2023-09-14
requires: 165, 712, 1155, 1271, 5267
---
## Abstract
...
/**
 * @notice Optional discovery extension for an asset profile.
 * @dev The implementing token contract also implements IERC7518.
 */
interface IERC7518AssetProfile is IERC165 {
  ...
  function assetKey(
    AssetRef calldata asset
  ) external view returns (bytes32 key);

  function assetProfile(
    AssetRef calldata asset
  ) external view returns (AssetProfile memory profile);
...
/**
 * @notice Optional discovery extension for named operational roles.
 * @dev The implementing token contract also implements IERC7518.
 */
interface IERC7518Roles is IERC165 {
  event RoleUpdated(
  ...
  function roleIdentity(
    bytes32 role,
    address account
  ) external view returns (bytes32 identityId);
}

/**
 * @notice Optional authorization extension for dynamic compliance vouchers.
 * @dev The implementing token contract also implements IERC7518.
 */
interface IERC7518Authorization is IERC165 {
  ...
  struct ComplianceAuthorization {
    address token;
    address authority;
  ...
  function authorizationHash(
    ComplianceAuthorization calldata authorization
...
/**
 * @notice Optional record extension for asset actions.
 * @dev The implementing token contract also implements IERC7518.
 */
interface IERC7518ActionRecord is IERC165 {
  ...
  function announceAction(ActionRecord calldata record) external;
  ...
  function actionRecord(
    bytes32 actionId
  ) external view returns (ActionRecord memory record);
}
```

### Conformance and interface detection

An `IERC7518` implementation MUST implement ERC 1155 and ERC 165. It MUST return `true` from `supportsInterface` for:

- `0xd9b67a26`, the ERC 1155 interface identifier.
- `0x5a264a5d`, the `IERC7518` interface identifier defined above.

Each extension below is OPTIONAL and has its own ERC 165 interface identifier. Support for one extension does not imply support for another.

- An implementation of `IERC7518AssetProfile` MUST return `true` for `0xe8aa3d31`.
- An implementation of `IERC7518Roles` MUST return `true` for `0x779da628`.
- An implementation of `IERC7518Authorization` MUST return `true` for `0x5b880cc8`.
- An implementation of `IERC7518ActionRecord` MUST return `true` for `0x9118aff3`.

An implementation of `IERC7518Authorization` MUST expose its EIP 712 domain through ERC 5267 and follow the authorization rules below. The `IERC7518` interface identifier remains `0x5a264a5d`.
...
### Asset Profile Extension

`IERC7518AssetProfile` provides discovery for the economic asset and its current policy. The extension is OPTIONAL.

`AssetRef` identifies a position by token contract, token identifier and partition identifier. An implementation MUST validate each reference before returning profile data. It MUST revert when a reference is invalid or unsupported.

`assetKey` MUST return:
...
The value is a chain local key for the referenced position. The same valid reference MUST return the same key while the chain identifier and token contract remain unchanged.

`assetProfile` MUST return a nonzero `assetId` for a recognized position. `assetId` identifies the economic asset across contract upgrades or chain representations. `policyId` identifies the current policy used for new authorizations. `profileHash` MUST equal the `keccak256` hash of the exact profile document bytes available through `profileURI`.

When any asset profile field changes, the implementation MUST emit `AssetProfileUpdated`. An implementation MUST NOT silently assign a different `assetId` to the same economic asset.

For `identifier`, `scheme` MUST equal `keccak256(bytes(canonicalSchemeName))`. A canonical scheme name SHOULD use uppercase ASCII such as `ISIN`, `DTI` or `CUSIP`. The function MUST return an empty string when a supported scheme has no assigned value.

### Roles Extension

`IERC7518Roles` provides discovery for named operational roles. The extension is OPTIONAL.

The following role keys are defined:

```solidity
keccak256("REGISTRAR")
keccak256("TRANSFER_AGENT")
keccak256("PAYING_AGENT")
keccak256("COMPLIANCE_AUTHORITY")
```

An implementation MAY define additional role keys. `roleHolder` MUST return `true` only while the account holds the named role. `roleIdentity` MUST return the identity reference for an active holder and MUST return `bytes32(0)` when the account does not hold the role.

Every role assignment, removal or identity reference change MUST emit `RoleUpdated`. The method used to change role state is implementation specific and MUST use access control. An identity reference SHOULD be opaque and MUST NOT require personal data onchain.

### Compliance Authorization Extension

`IERC7518Authorization` defines a typed compliance voucher with validation, replay state and revocation. The extension is OPTIONAL.

#### Typed authorization

The EIP 712 type string MUST be:

```text
ComplianceAuthorization(address token,address authority,bytes32 operationHash,bytes32 policyId,bytes32 contextHash,bytes32 nonce,uint48 validAfter,uint48 validBefore)
```

`authorizationHash` MUST return the EIP 712 digest for this value. The domain MUST bind the authorization to the current `chainId` and the implementing token as `verifyingContract`. The domain MUST be discoverable through ERC 5267.

The fields have the following meaning:

- `token` is the token contract that may execute the authorized operation. It MUST equal `address(this)`.
- `authority` is the approved signer for the authorization.
- `operationHash` binds the authorization to one exact token operation.
- `policyId` identifies the policy or rule set used by the authority.
- `contextHash` commits to application specific compliance context.
- `nonce` provides uniqueness within the signed authorization.
- `validAfter` is the first timestamp at which the authorization is valid, inclusive.
- `validBefore` is the timestamp at which the authorization expires, exclusive.

`validateAuthorization` MUST return `true` only when all of the following are true:

- The authorization is for the implementing token.
- `block.timestamp` is greater than or equal to `validAfter` and less than `validBefore`.
- `authorizationState(authorizationHash(authorization))` is `Unused`.
- `authority` is approved by the implementation access control policy.
- The signature is valid for `authority` and the EIP 712 digest.

If `authority` is a contract, signature validation MUST use ERC 1271. The function MUST NOT change state and MUST return `false` for an invalid, expired, consumed or revoked authorization.

#### Canonical operation hashes

For `safeTransferFrom`, `operationHash` MUST be calculated from:

```text
ERC7518SingleTransfer(address operator,address from,address to,uint256 id,uint256 amount,bytes32 applicationDataHash)
```

For `safeBatchTransferFrom`, `operationHash` MUST be calculated from:

```text
ERC7518BatchTransfer(address operator,address from,address to,bytes32 idsHash,bytes32 amountsHash,bytes32 applicationDataHash)
```

The hashes MUST be calculated as follows:

```solidity
bytes32 singleOperationHash = keccak256(
  abi.encode(
    keccak256("ERC7518SingleTransfer(address operator,address from,address to,uint256 id,uint256 amount,bytes32 applicationDataHash)"),
    msg.sender,
    from,
    to,
    id,
    amount,
    keccak256(applicationData)
  )
);

bytes32 batchOperationHash = keccak256(
  abi.encode(
    keccak256("ERC7518BatchTransfer(address operator,address from,address to,bytes32 idsHash,bytes32 amountsHash,bytes32 applicationDataHash)"),
    msg.sender,
    from,
    to,
...
Binding the operator, parties, asset identifiers, amounts and application data prevents use with another operation.

#### Transfer data envelope

An authorized transfer MUST place the voucher in the current ERC 1155 `data` parameter. The standardized envelope is:
...
The first value is the ERC 7518 authorization magic. When this magic is absent, `data` remains application defined. When it is present, an implementation of `IERC7518Authorization` MUST decode the remaining values as `ComplianceAuthorization`, `bytes` and `bytes`. The complete original `data` value MUST be passed unaltered to the ERC 1155 receiver hook.

#### Atomic use and revocation

A state changing transfer using this envelope MUST:

1. Recalculate and compare the canonical `operationHash`.
2. Validate the authorization and signature.
3. Mark the authorization hash as `Consumed`.
4. Emit `AuthorizationConsumed`.
5. Complete the transfer.

Consumption MUST occur in the same transaction as the transfer and before any external receiver hook. If the transfer reverts, the state change and event are reverted. Authorization consumption is available only through transfer execution.

`revokeAuthorization` MUST be access controlled. It MUST mark an `Unused` authorization hash as `Revoked` and emit `AuthorizationRevoked`. A `Consumed` authorization MUST remain `Consumed` and both terminal states MUST remain final.

### Action Record Extension

`IERC7518ActionRecord` provides records for asset actions such as splits, tenders or votes. The extension is OPTIONAL.
...
`announceAction` MUST require a nonzero action identifier that has not been used. It MUST store the record with status `Announced` and emit `ActionAnnounced`.

After announcement, `actionId`, `actionType`, `assetId`, `assetKey`, `recordTime`, `effectiveTime`, `termsHash` and `termsURI` MUST remain unchanged. `recordTime` and `effectiveTime` MUST use Unix time in seconds. `termsHash` MUST equal the `keccak256` hash of the exact terms document bytes available through `termsURI`.

`setActionStatus` MUST emit `ActionStatusChanged`. A completed or cancelled action MUST NOT return to announced or active status. State changing functions in this extension MUST use access control.

`actionRecord` MUST return status `Unknown` for an action identifier that has not been announced. It MUST NOT return an announced record with a zero action identifier.

### Optional Extension Composition

When `IERC7518AssetProfile` and `IERC7518Authorization` are both implemented, an authorization `policyId` MUST equal the current asset profile `policyId` when the authorization is consumed.

When `IERC7518Roles` and `IERC7518Authorization` are both implemented, an authorization authority MUST hold the `COMPLIANCE_AUTHORITY` role when the authorization is consumed.

When `IERC7518AssetProfile` and `IERC7518ActionRecord` are both implemented, the action `assetKey` and `assetId` MUST match the current asset profile when the action is announced.
...
#### Mapping Other Token Standards

An [ERC 20](./eip-20.md) issuance MAY map its complete fungible supply to one ERC 7518 partition. The default mapping SHOULD use `tokenId = 0`. The source contract address, source chain and mapped ERC 7518 contract MUST identify the partition.

An [ERC 721](./eip-721.md) adapter MAY map each source `tokenId` to one ERC 7518 partition. An [ERC 1155](./eip-1155.md) adapter SHOULD map each source `tokenId` to the equivalent partition when the supply and rights are equivalent.

The adapter or wrapper MAY implement any optional extension on the mapped ERC 7518 path.
...
The authorization extension defines a typed approval for one transfer. It binds the token, authority, operation, policy reference, context, nonce and validity period. The authorization is verified and consumed in the transfer transaction to prevent replay.
...
Implementations MAY expose `IERC7518AssetProfile`, `IERC7518Roles`, `IERC7518Authorization` and `IERC7518ActionRecord` through ERC 165. Each extension is independent. Authorized transfers use the current ERC 1155 `data` parameter. Existing calls continue to follow the current transfer interface.
...
8. Authorization Authority: Authorization signers can approve regulated transfers. Implementations MUST protect signer keys, restrict approved authorities and support prompt revocation of unused authorizations.
9. Replay Protection: Implementations MUST verify the EIP 712 domain, validity window, operation hash and authorization state. The authorization MUST be marked consumed before the ERC 1155 receiver hook. A reverted transfer also reverts consumption.
10. Contract Signers: ERC 1271 validation can call untrusted code. Implementations SHOULD limit gas where appropriate, handle invalid return values and prevent reentrancy from changing authorization or transfer state.
11. Cross Chain Use: Domain binding prevents an authorization signed for one chain or token from being replayed on another. Each deployment MUST enforce its local policy, balances, freezes and revocations.
12. Revocation Access: `revokeAuthorization` MUST use explicit access control. Revocation reason codes are identifiers for the related offchain process.
13. Rejection Observability: Implementations SHOULD expose rejection details through simulation or application logs because reverted events are not retained onchain.
14. Profile Integrity: Clients MUST verify `profileHash` before relying on profile document bytes. Implementations SHOULD use stable retrieval locations and preserve prior profile events for audit.
15. Role Administration: Role state changes MUST use explicit access control. Identity references SHOULD avoid personal data and SHOULD reveal no more information than required.
16. Action Record Integrity: Implementations MUST restrict action creation and status changes. Completed or cancelled records MUST remain terminal and announced terms MUST remain unchanged.
